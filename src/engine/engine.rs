...
impl Engine {
    ...
    pub fn add_cube_to_scene(&mut self, translation: Option<Vec3>) -> () {
        match translation {
            Some(translation) => {
                let mut cube = Box::new(Cube::new(Vec3{ x: 0.2, y: 0.2, z: 0.2 }, Transform { translation, ..Default::default()}));
            ...
            None => {
                let rand_x: f32 = rand::thread_rng().gen_range(-3_f32..3_f32);
                let rand_y: f32 = rand::thread_rng().gen_range(-3_f32..1_f32);
                let rand_z: f32 = rand::thread_rng().gen_range(-4_f32..-2_f32);
    ...
    pub fn work_off_event_queue(&mut self, renderer: & mut Renderer, frame_in_flight_index: usize) {
        ...
        for _ in 0..len {
            match self.event_queue.pop() { // ToDo: decide if fifo or lifo is the right way, for now lifo seems to work
                Some(EngineEvent::EntityAdded(entity_transform, entity_mesh, entity_index)) => renderer.entity_added_handler(entity_transform, entity_mesh, entity_index, frame_in_flight_index),
